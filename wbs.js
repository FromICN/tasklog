...
function wbsEmptyCol(colCls) { return '<span class="wbs-col ' + (colCls || '') + '"></span>'; }

// STATUS 셀 래퍼 (컬럼 폭 조정용)
function wbsStatusCell(badgeHtml) { return '<span class="wbs-col-status-cell">' + badgeHtml + '</span>'; }
...
// ── Linked Task (연계 TASK: 선행 ← / 후행 →) ──
//  상세 패널의 '🔗 연계 TASK'(prevTaskIds · nextTaskIds)를 트리에서도 바로 본다.
//  이름은 Task 목록 표기와 같게 [YYMMDD] 접두어를 떼고 보여준다.
function wbsLinkedName(id) {
...
function wbsStartCol(task) {
  if (!task.startDate) return wbsEmptyCol('wbs-col-start');
...
function wbsRefreshTree() {
  ...
  if (root) { root.innerHTML = buildWbsTree(); wbsAttachColResize(); }
}

// 헤더 셀: 항목 텍스트(셀) 자체 클릭 = 필터 드롭다운, 오른쪽 ▲▼ = 정렬
...
// 컬럼 너비 드래그 조정 (Linked Task / Start / Due / Status — localStorage 저장)
function wbsAttachColResize() {
  var root = document.getElementById('wbs-root');
...
//  색으로 상태를 구분한다 — 대기 회색 · 진행 초록 · 완료 파랑 · 중단 빨강.
//  값은 theme.css 의 --st-* 토큰(라이트/다크 각각 정의)을 쓴다.
var WBS_STATUS_COLORS = {
  '대기': 'var(--st-wait)',
  '진행': 'var(--st-run)',
  '중단': 'var(--st-stop)',
  '완료': 'var(--st-done)',
...
function wbsStatusBadge(label) {
  var c = WBS_STATUS_COLORS[label] || 'var(--st-wait)';
...
function renderWbsCoreGroup(nodeId, label, actionGroups, lk, sgId) {
  if (_wbsOpen[nodeId] === undefined) _wbsOpen[nodeId] = true;
  var open = _wbsOpen[nodeId];
  var allTasksEarly = [].concat.apply([], Object.values(actionGroups));
  var firstT = allTasksEarly[0] || null;
  var dropYear = wbsTaskYear(firstT);
  var sgText = (firstT && firstT.mdtGoal && firstT.mdtGoal.text) ? firstT.mdtGoal.text : '';
  var dropAttrs = ' data-wbs-drop-sgid="' + (sgId != null ? sgId : '') + '"'
    + ' data-wbs-drop-year="' + (dropYear != null ? dropYear : '') + '"'
    + ' data-wbs-drop-sgtext="' + encodeURIComponent(sgText || '') + '"';
  var inner = '';
  if (actionGroups['_']) inner += renderWbsActionGroup(nodeId + '-none', 'Project 미연결', actionGroups['_'], lk, '_', '', sgId, dropYear, sgText);
  Object.keys(actionGroups).filter(function(k){ return k !== '_'; }).forEach(function(ak) {
    ...
    inner += renderWbsActionGroup(nodeId + '-' + ak, akText, taskArr, lk, ak, akText, sgId, dropYear, sgText);
  });
  var allTasks = allTasksEarly;
...
function renderWbsActionGroup(nodeId, label, taskArr, lk, ak, akText, sgId, dropYear, sgText) {
  if (_wbsOpen[nodeId] === undefined) _wbsOpen[nodeId] = true;
  var open = _wbsOpen[nodeId];
...
function wbsAssignTaskSection(task, sgId, dropYear, sgText, lk) {
  if (typeof loadLifeWheel === 'function') loadLifeWheel();
  var hasLk = (lk !== null && lk !== undefined && lk !== '_' && lk !== '');
  if (sgId == null && !hasLk) {
    // 미분류(Section 없음)로 이동
  ...
  // 라이프휠 섹션 인덱스: lk 우선, 없으면 sgId-1
  var idx = hasLk ? parseInt(lk) : (sgId != null ? sgId - 1 : null);
  var secs = (typeof getLwSections === 'function') ? getLwSections() : [];
  var sec = (secs && idx != null) ? secs[idx] : null;
  if (idx != null) {
    task.lwSection = idx;
    task.lwSectionName  = sec ? sec.name  : '';
    task.lwSectionEmoji = sec ? sec.emoji : '';
  }
  // 만다라트 목표(Section): 드롭 대상의 실제 sgId + 연도로 정확히 배정 (인덱스 추정 금지)
  var year = (dropYear != null) ? dropYear
           : ((typeof lwCurrentYear !== 'undefined' && lwCurrentYear) ? lwCurrentYear : new Date().getFullYear());
  var effSgId = (sgId != null) ? sgId : (idx != null ? idx + 1 : null);
  var mdt = (typeof getMdt === 'function') ? getMdt(year) : null;
  var sg  = (mdt && effSgId != null) ? (mdt.subGoals || []).find(function(s){ return s.id === effSgId; }) : null;
  if (effSgId != null) {
    task.mdtGoal = { year: year, sgId: effSgId, text: (sg && sg.text) ? sg.text : (sgText || '') };
  } else {
    delete task.mdtGoal;
  }
...
function wbsAssignTaskAction(task, sgId, dropYear, sgText, lk, actionId, actionText) {
  wbsAssignTaskSection(task, sgId, dropYear, sgText, lk);
  if (actionId === null) { delete task.mdtAction; return; }
  var year = (dropYear != null) ? dropYear
           : ((typeof lwCurrentYear !== 'undefined' && lwCurrentYear) ? lwCurrentYear : new Date().getFullYear());
  var useSgId = (task.mdtGoal && task.mdtGoal.sgId != null) ? task.mdtGoal.sgId : sgId;
  task.mdtAction = { year: year, sgId: useSgId, actionId: actionId, text: actionText || '' };
}
...
function wbsHandleTaskDrop(taskId, dropEl, isCopy) {
  ...
  var sgIdRaw = dropEl.dataset.wbsDropSgid;
  var sgId = (sgIdRaw && sgIdRaw !== '' && sgIdRaw !== 'null' && sgIdRaw !== '_') ? parseInt(sgIdRaw) : null;
  var dropYear = dropEl.dataset.wbsDropYear ? parseInt(dropEl.dataset.wbsDropYear) : null;
  var sgText = dropEl.dataset.wbsDropSgtext ? decodeURIComponent(dropEl.dataset.wbsDropSgtext) : '';
  ...
  if (dropEl.dataset.wbsDropAk !== undefined) {
    ...
    wbsAssignTaskAction(target, sgId, dropYear, sgText, lk, ak, akText);
  } else {
    wbsAssignTaskSection(target, sgId, dropYear, sgText, lk);
  }
